...
static void first_session(pam_handle_t* pamh, entry* ent, struct passwd* pwd) {
    if (pwd->pw_uid != 1000)
        return; // FIXME
    ...
    try {
        if (!ent->hive_mounted) {
            ...
            ent->hive_mounted = true;
        }

        if (!ent->classes_mounted) {
...
int pam_sm_open_session(pam_handle_t* pamh, __attribute__((unused)) int flags,
    ...
    if (!found) {
        ...
        ent->uid = pwd->pw_uid;
        ent->num_sessions = 1;
        ent->hive_mounted = false;
        ent->classes_mounted = false;

        first_session(pamh, ent, pwd);
...
static void last_session(pam_handle_t* pamh, entry* ent, struct passwd* pwd) {
    if (pwd->pw_uid != 1000)
        return; // FIXME
